...
namespace Chapter_6_CSharp
{
    public partial class Aggregates : Form
    {
        ...
        private void ActCompute_Click(System.Object sender, System.EventArgs e)
        {
            ...
            expression = ComputeFunction.SelectedItem.ToString() + "(" + columnName + ")";
            ...
            try { result = whichTable.Compute(expression, ""); }
            catch (Exception ex) { MessageBox.Show("Could not compute the column: " + ex.Message); return; }
            // ----- Display the results.
            if (DBNull.Value.Equals(result)) MessageBox.Show("NULL");
